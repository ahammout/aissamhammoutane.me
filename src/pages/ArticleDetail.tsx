      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/blog" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span className="font-medium text-sm">{t("blog.backToArticles")}</span>
          </Link>
          <ThemeToggle />
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto">
          <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <header className="mb-12">
              ...
              <h1 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold mb-6 leading-tight">{article.title}</h1>
              ...
              <div className="space-y-6 text-foreground/90 leading-relaxed">
                {article.content.split("\n").map((paragraph, index) => {
                  ...
                  if (trimmed.startsWith("# ")) {
                    return <h1 key={index} className="font-display text-3xl font-bold mt-8 mb-4 text-foreground">{trimmed.replace("# ", "")}</h1>;
                  }
                  if (trimmed.startsWith("## ")) {
                    return <h2 key={index} className="font-display text-2xl font-semibold mt-8 mb-4 text-foreground">{trimmed.replace("## ", "")}</h2>;
                  }
                  ...
                  return <p key={index} className="text-foreground/80" dangerouslySetInnerHTML={{ __html: formattedParagraph }} />;
                })}
              </div>
            </div>
          </motion.article>
